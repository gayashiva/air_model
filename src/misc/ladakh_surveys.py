...
if __name__ == "__main__":
    ...
    #                    ]
    legend_elements = [
                       Line2D([0], [0], linestyle= '--', lw=1, color='r', label=text),
                       ]

    fig, ax = plt.subplots()
    plt.plot(x,p(x),"r--")
    df = df[df.Winter == '2020']
    print(df)
    ax.scatter(df.Altitude,df.Volume,s=20, color=mypal[1], marker='.')
    ...
    ax.legend(handles=legend_elements, prop={"size": 8}, loc = 'best', title='Trend line')
