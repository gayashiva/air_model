...
class Icestupa:
    ...
    def __init__(self, *initial_data, **kwargs):
        ...
        #self.raw = os.path.join(dirname, "data/" + self.name + "/raw/")
        #self.input = os.path.join(dirname, "data/" + self.name + "/interim/")
        #self.output = os.path.join(dirname, "data/" + self.name + "/processed/")
    ...
    def calibration(self, site):
        ...
        if site in ['guttannen']:
            df_c = pd.read_csv(self.input + self.name + "_drone.csv", sep=",", header=0, parse_dates=["When"])
    ...
    def read_input(self): # Use processed input dataset
        ...
        self.df = pd.read_hdf(
            self.input + "model_input_" + self.trigger + ".h5", "df"
        )
...
def summary_figures(icestupa):

    # Class variables used
    df = icestupa.df
    trigger = icestupa.trigger
    RHO_I = icestupa.RHO_I
    TIME_STEP = icestupa.TIME_STEP

    output = icestupa.output
    ...
    lns3 = ax3.plot(
        x, df.LW_in, linestyle="-", label="Longwave", color=CB91_Green
    )
    ax3.axvspan(
        df.When.head(1).values,
        df.When.tail(1).values,
        facecolor="grey",
        alpha=0.25,
    )
    ...
    fig.autofmt_xdate()
    plt.savefig(
        output+ "paper_figures/Model_Input_" + trigger + ".jpg", dpi=300, bbox_inches="tight"
    )
    plt.clf()
    ...
    with pd.option_context("mode.chained_assignment", None):
        for i in range(1, dfds.shape[0]):
            ...
            dfds.loc[i, "melted"] *= -1 / (df.loc[i, "SA"] * RHO_I)
            dfds.loc[i, "gas"] *= -1 / (df.loc[i, "SA"] * RHO_I)
            dfds.loc[i, "ppt"] *= 1 / (df.loc[i, "SA"] * RHO_I)
            dfds.loc[i, "dpt"] *= 1 / (df.loc[i, "SA"] * RHO_I)
            dfds.loc[i, "cdt"] *= 1 / (df.loc[i, "SA"] * RHO_I)
    ...
    plt.tight_layout()
    plt.savefig(
        output + "paper_figures/Model_Output_" + trigger + ".jpg", dpi=300, bbox_inches="tight"
    )
    plt.clf()
