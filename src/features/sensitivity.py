...
def icestupa_sim(index, X, df_in, q):
    ...
    max_melt_thickness = float(df["thickness"].min())

    # del [[df_in, df]]

    q.put((index, Max_IceV, Efficiency, max_melt_thickness))
    # return ((index, Max_IceV, Efficiency, max_melt_thickness))
...
if __name__ == '__main__':
    ...
    F = np.zeros([param_values.shape[0]])
    I = np.zeros([param_values.shape[0]])

    for i in range(param_values.shape[0]):
        I[i], Y[i], Z[i], F[i] = q.get()   # Returns output
    ...
    for j in I:
        ...
        dfo.loc[j, "max_melt_thickness"] = F[j] * 1000
    ...
    #
    # for j, X in enumerate(param_values):
    #     I[j], Y[j], Z[j], F[j] = icestupa_sim(j, X)
    #     dfo.loc[j, "dx"] = param_values[j][0] * 1000
    #     dfo.loc[j, "Max_IceV"] = Y[j]
    #     dfo.loc[j, "Efficiency"] = Z[j] * 100
    #     dfo.loc[j, "max_melt_thickness"] = F[j] * 1000
